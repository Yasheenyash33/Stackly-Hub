...
create_user = crud_module.create_user
create_session = crud_module.create_session
...
def create_sample_users(db: Session):
    ...
    admin, temp_password = create_user(db, admin_data)
    print(f"Created admin user: {admin.username} (temp password: {temp_password})")
    ...
    for trainer_data in trainers_data:
        trainer, temp_password = create_user(db, trainer_data)
        trainers.append(trainer)
        print(f"Created trainer: {trainer.username} (temp password: {temp_password})")
    ...
    for trainee_data in trainees_data:
        trainee, temp_password = create_user(db, trainee_data)
        trainees.append(trainee)
        print(f"Created trainee: {trainee.username} (temp password: {temp_password})")
...
def main():
    ...
    db.execute(text("DELETE FROM password_change_logs"))
    db.execute(text("DELETE FROM attendance"))
    db.execute(text("DELETE FROM session_trainees"))
    db.execute(text("DELETE FROM sessions"))
    db.execute(text("DELETE FROM assigned_students"))
